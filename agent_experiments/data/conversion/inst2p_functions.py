...
def context_and_reduced_casino_cust_format(inst):
    system_msg = {
        'role': 'system',
        'content': 
'''You are assisting the user in annotating utterances in a negotiation for dividing 3 units each of food, water, and firewood. 
...
"unknown" - the utterance does not fit any of the above labels.'''
    ...
    get_annot_req = lambda u: f'The speaker\'s highest priority item is {inst["participant_info"][u["id"]]["value2issue"]["High"]} and their lowest priority item is {inst["participant_info"][u["id"]]["value2issue"]["Low"]}. What is the annotation for this utterance? "{u["text"]}"'

    return [[system_msg, {'role': 'user', 'content': get_annot_req(u)}] for u in inst['chat_logs']]

def diahist_and_reduced_casino_cust_format(inst):
    system_msg = {
        'role': 'system',
        'content': 
'''You are assisting the user in annotating utterances in a negotiation for dividing 3 units each of food, water, and firewood. 
...
"unknown" - the utterance does not fit any of the above labels.'''
    ...
    get_annot_req = lambda u: f'What is the annotation for this utterance? "{u["text"]}"'

    get_annot_req_dia_hist = lambda u, r: f'The utterance the speaker is responding to is "{r}". What is the annotation for this utterance? "{u["text"]}"'

    return [[system_msg, {'role': 'user', 'content': get_annot_req(inst['chat_logs'][0])}]] + [[system_msg, {'role': 'user', 'content': get_annot_req_dia_hist(u, inst['chat_logs'][i]['text'])}] for i, u in enumerate(inst['chat_logs'][1:])]


def context_plus_diahist_and_reduced_casino_cust_format(inst):
    system_msg = {
        'role': 'system',
        'content': 
'''You are assisting the user in annotating utterances in a negotiation for dividing 3 units each of food, water, and firewood. 
...
"unknown" - the utterance does not fit any of the above labels.'''
    ...
    get_annot_req_dia_hist = lambda u, r: f'The speaker\'s highest priority item is {inst["participant_info"][u["id"]]["value2issue"]["High"]} and their lowest priority item is {inst["participant_info"][u["id"]]["value2issue"]["Low"]}. The utterance the speaker is responding to is "{r}". What is the annotation for this utterance? "{u["text"]}"'
