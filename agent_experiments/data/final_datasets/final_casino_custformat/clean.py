...
def slot_getter(annot_iterator, labels_list, items_list):
    ...
    while cur:
        valid = False
        for i in items_list:
            if cur.startswith(i) and not (cur.endswith('<slot>')):
...
def main(in_file: str, out_file: str, dataset: str = 'casino'):
    # Get proper labels
    if dataset == 'dnd':
        items = DND_ITEMS
        labels = DND_LABELS
    ...
    with open(out_file, 'w') as f:
        for line in lines:
            ...
            dialogue = tmp[0].\
                replace('empathy coordination', 'empathycoordination').\
                replace('no need', 'noneed').\
                replace('elicit preference', 'elicitpreference').\
                replace('vouch fairness', 'vouchfairness').\
            ...
            # TODO: Try leaving as single tokens. makes more sense for lstm?
            '''
            dialogue = dialogue.\
                replace('empathycoordination', 'empathy coordination').\
                replace('noneed', 'no need').\
                replace('elicitpreference', 'elicit preference').\
                replace('vouchfairness', 'vouch fairness').\
                replace('expresspreference', 'express preference')
            '''
