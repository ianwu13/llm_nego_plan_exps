...
# DATASET SPECIFIC #############################################
CTX_PAIRS_FILE = "misc/flask_utils/mod_cxt_pairs_dnd.json"
...
GRU_FILES = [
    "dnd_supervised_30ep.pt",
    "dnd_rl_selfish_4ep_rw_own_points.pt",
    "dnd_rl_fair_4ep_rw_combine50_50.pt",
]
GRU_MODELS = []
# DATASET SPECIFIC ##################################################
...
def get_model(name):
    ...
    if agent_type == 'llm_no_planning':
        ...
        if template_gen:
            template_funct = get_template_funct('dnd')
        else:
            template_funct = None
