...
def dnd_a2u_prompt(inst):
    ...
    if len(inst['dialogue']) == 0:
        ...
        dialogue_str = f" The negotiation dialogue up to this point is:\n{' '.join(inst['dialogue'])}\n"

        preprocessed_dia_acts = list(filter(lambda s: s != '<eos>', inst['dia_acts']))
        last_you = len(preprocessed_dia_acts) - 1 - preprocessed_dia_acts[::-1].index('YOU:')
        preprocessed_dia_acts = preprocessed_dia_acts[:last_you]
        da_str = f" The dialogue act annotations up to this point are:\n{' '.join(preprocessed_dia_acts)}\n"
